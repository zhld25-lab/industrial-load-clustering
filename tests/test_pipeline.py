...
from industrial_load.dtw import dtw_distance, dtw_reassign
...
def test_dtw_identity_and_symmetry():
    ...
    assert dtw_distance(np.array([0.0, 1.0]), np.array([0.0, 0.5, 1.0]), window=1) >= 0


def test_dtw_reassignment_returns_valid_clusters():
    curves = np.array([[0, 0, 1], [0, 1, 1], [5, 5, 4], [5, 4, 4]], dtype=float)
    labels = np.array([0, 0, 1, 1])
    reassigned = dtw_reassign(curves, labels, n_clusters=3, window=1)
    assert reassigned.shape == labels.shape
    assert set(reassigned).issubset({0, 1, 2})
